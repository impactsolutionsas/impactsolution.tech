...
import { SolutionMediaSlider } from "./solution-media-slider";
import {
  ArrowUpRight,
  CheckCircle2,
  AlertTriangle,
...
export interface SolutionValueItem {
  title: string;
  body: string;
  /** When true, this card renders with a persistent accent liseret to highlight
   *  the flagship module (e.g. AMANET's "11-module ERP platform"). */
  featured?: boolean;
}
...
export interface SolutionInnovationItem {
  title: string;
  body: string;
}
...
interface SolutionDetailProps {
  ...
  /** Optional differentiator block ("What makes us unique") — rendered between Value and Impact. */
  innovationLabel?: string;
  innovation?: SolutionInnovationItem[];
  ...
  /** Optional initial markets — rendered as a chip row under Users. */
  marketsLabel?: string;
  markets?: string[];
  ...
  /** Optional hero media (images + YouTube videos) displayed in a split layout beside the title. */
  media?: MediaItem[];
}
...
export function SolutionDetail({
  ...
  const hasInnovation = innovation && innovation.length > 0 && innovationLabel;
  const hasMarkets = markets && markets.length > 0 && marketsLabel;
  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-24 sm:px-6 sm:py-28 lg:px-8">
        {/* Identity — split hero when media is provided */}
        <div className={hasMedia ? "grid gap-8 lg:grid-cols-12 lg:items-center" : "flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between"}>
          <div className={hasMedia ? "lg:col-span-7 flex flex-col gap-5" : "flex flex-col gap-5"}>
          ...
          {hasMedia ? (
            <div className="order-first lg:order-last lg:col-span-5">
              <SolutionMediaSlider media={media!} accent={accent} name={name} />
            </div>
          ) : (
        ...
        {/* Innovation (optional differentiator) */}
        {hasInnovation && (
          <div className="mt-20">
            <div className="flex items-center gap-2">
              <Lightbulb
            ...
            <div
              className="relative mt-5 overflow-hidden rounded-3xl border border-border p-6 sm:p-8"
              style={{
                backgroundImage: `linear-gradient(135deg, ${accent}0F, ${accent}03 55%, transparent)`,
              }}
            >
              <div
                aria-hidden
              ...
              <div className="mt-5">
                <div className="flex items-center gap-2">
                  <MapPin
                    ...
                    strokeWidth={1.75}
                  />
                  <span className="text-[0.65rem] font-semibold tracking-[0.14em] uppercase text-muted-foreground">
                    {marketsLabel}
                  </span>
                </div>
                <ul className="mt-2.5 flex flex-wrap gap-1.5">
                  {markets!.map((m) => (
                    <li
                      key={m}
                      className="rounded-full border border-border bg-background px-2.5 py-1 text-[0.7rem] font-medium text-foreground/70"
                    >
                      {m}
                    </li>
                  ))}
        ...
        {/* Discover CTA — internal Link for /solutions/*, plain <a> for external product sites. */}
        <div className="mt-14">
          {isExternalHref ? (
            <a
              href={href}
